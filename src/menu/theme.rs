...
/// Screen ready to render; back row already added.
...
fn metrics(style: &Style, w: i32, h: i32) -> Metrics {
    match style {
        Style::Title { .. } => {
            let fs = 28;
            Metrics { fs, line_h: fs + 14, start_y: h / 2 - (fs + 14), x: 0, centered: true }
        }
        Style::Panel => {
            let fs = 26;
            Metrics { fs, line_h: fs + 20, start_y: h / 4 + 20, x: w / 2 - 260, centered: false }
        }
    }
}

fn default_layout(v: &PresentedView, w: i32, h: i32) -> Vec<RowRect> {
    let m = metrics(&v.style, w, h);
    v.rows
        .iter()
        .enumerate()
        .map(|(i, _)| RowRect {
...
fn draw_panel(f: &mut Frame, v: &PresentedView, sel: usize, w: i32, h: i32) {
    ...
    for (i, row) in v.rows.iter().enumerate() {
        ...
        if let Some(detail) = &row.detail {
            shadowed(f, detail, m.x + 40, y + m.fs + 2, 16, Color::DARKGRAY);
        }
    }
}
