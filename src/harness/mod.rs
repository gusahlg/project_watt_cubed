...
/// harness derives `Camera3D` through the SAME `camera_with_fov` path the game uses.
...
impl CameraPose {
    /// Derive the render camera the way the game does: build a `Player` at this
    /// pose and go through `Player::camera_with_fov` — the harness owns no
    /// projection math of its own.
    pub fn camera(&self, fovy: f32) -> Camera3D {
        let mut p = Player::new(self.pos);
        p.yaw = self.yaw;
        p.pitch = self.pitch;
        p.camera_with_fov(fovy)
    }
}
...
fn execute(stages: Vec<Stage>) -> Outcomes {
    ...
    voxel_engine::run(scripted_config(), move |eng| {
        ...
        let g = game.get_or_insert_with(|| {
            let mut g = Game::scripted(stage.seed);
        ...
        g.update(eng, &mut mods, &mut settings);
...
// ============================================================================
// Reference-space verdict — capture rig, deleted with the verdict
// ============================================================================

/// Capture the verdict matrix column for one variant: the four verdict
/// shots under the NEW_SHOKA palette, written to
/// `screenshots/verdict/<tag>_<shot>.png` (no criteria — these are for
/// side-by-side adjudication, never ImageMatch).
///
/// `tag` selects the palette: `v2` = display-decoded literals
/// ([`palette::new_shoka_v2`]); `v1`/`v3` = the linear import as it stands.
/// V3's tonemap half is the engine's `WATT_TONEMAP=makeup` gate, which the
/// `golden` bin sets from the same tag. Returns how many captures failed.
pub fn run_verdict(tag: &str) -> usize {
    const VERDICT_SHOTS: [&str; 4] =
        ["spawn_forward", "horizon_fog_vs_sky", "night_field", "cave_interior"];
    let dir = PathBuf::from("screenshots/verdict");
    std::fs::create_dir_all(&dir).expect("create screenshots/verdict");
    let v2 = tag == "v2";
    let stages: Vec<Stage> = golden_shots()
        .into_iter()
        .filter(|s| VERDICT_SHOTS.contains(&s.name))
        .map(|s| Stage {
            seed: s.seed,
            cam: Some(s.cam),
            day: s.day,
            view: DebugView::Normal,
            // The only per-shot setup among the verdict shots is the cave carve,
            // so the palette pin composes with it by name, not by combinator.
            setup: Some(match (v2, s.setup.is_some()) {
                (false, false) => set_shoka,
                (false, true) => set_shoka_carved,
                (true, false) => set_shoka_v2,
                (true, true) => set_shoka_v2_carved,
            }),
            kind: StageKind::Capture { path: dir.join(format!("{tag}_{}.png", s.name)) },
        })
        .collect();
    let out = execute(stages);
    let mut failed = 0;
    for (path, r) in &out.captures {
        match r {
            Ok(()) => println!("verdict: wrote {}", path.display()),
            Err(e) => {
                failed += 1;
                eprintln!("verdict: FAILED {} — {e}", path.display());
            }
        }
    }
    failed
...
fn set_shoka(g: &mut Game) {
    g.set_palette(crate::sky::palette::NEW_SHOKA);
}
fn set_shoka_v2(g: &mut Game) {
    g.set_palette(crate::sky::palette::new_shoka_v2());
}
fn set_shoka_carved(g: &mut Game) {
    set_shoka(g);
    carve_cave(g);
}
fn set_shoka_v2_carved(g: &mut Game) {
    set_shoka_v2(g);
    carve_cave(g);
}
...
pub fn golden_shots() -> Vec<GoldenShot> {
    vec![
        ...
        GoldenShot {
            ...
            // dropped 12 m into a carved room, facing the +x emitter (yaw 0).
            // `carve_cave` builds the room + light.
            cam: CameraPose { pos: DVec3::new(0.0, 68.0, 0.0), yaw: 0.0, pitch: 0.0 },
            name: "cave_interior",
            day: 0.5,
...
/// around the (already-teleported) camera and seat one emitter block on the +x
/// wall it faces (yaw 0). Centring on the live player position keeps the room in
/// step with the shot's pose with no duplicated coordinates. The edits land in
/// the world overlay before streaming, so the carved chunks generate already-carved.
