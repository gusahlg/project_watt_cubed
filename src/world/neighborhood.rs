...
/// Max buffers held per thread, per cell type. Small: workers are long-lived
/// and few (≤3), and a single job holds at most one live neighbourhood at a time.
const POOL_CAP: usize = 4;

/// A cell type that owns a thread-local free list of halo buffers. A
/// `thread_local!` static cannot name a generic `T`, so the per-type pool lives
/// behind this trait — one concrete monomorphic pool per implementor, keeping
/// BlockId and Lumel free lists isolated. Generic recycle policy (cap, reuse)
/// stays in the generic methods; an impl only hands out its pool. Written by
/// `pooled_cell!`.
pub trait Pooled: Copy + 'static {
    fn with_pool<R>(f: impl FnOnce(&mut Vec<Box<[Self]>>) -> R) -> R;
}

/// One `impl Pooled` = one thread-local buffer free list for `$t`.
macro_rules! pooled_cell {
    ($t:ty) => {
        impl $crate::world::neighborhood::Pooled for $t {
            fn with_pool<R>(f: impl FnOnce(&mut Vec<Box<[$t]>>) -> R) -> R {
                thread_local! {
                    static POOL: RefCell<Vec<Box<[$t]>>> = const { RefCell::new(Vec::new()) };
                }
                POOL.with_borrow_mut(f)
            }
        }
    };
}
