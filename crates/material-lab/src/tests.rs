...
fn cascade_last_write_is_position_order() {
    let law = Law::v0();
    // 50 units on one axis sits in v0's attractive band; NewContact still steps.
    let p = Configuration::single(Element([40, 40, 40, 40]));
    let t = Configuration::single(Element([90, 40, 40, 40]));
    let r = Configuration::single(Element([40, 90, 40, 40]));
    let mut grid = Grid::new(4, Configuration::void());
