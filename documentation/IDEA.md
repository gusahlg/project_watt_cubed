# Project Watt Cubed Pitch

Project Watt Cubed is a voxel sandbox game set in an infinite voxel world. Matter is configurations of a resource lattice under one integer law (see `documentation/material-model.md`).
The world is infinite in all three axis so you can dig infinitely deep and go infinitely high as well, maximum freedom.

The game is about:

* Exploring an infinite world
* Building cool systems
* Constructing beautiful buildings
* Programming complex machines
* Automating tedious work

## The Human Layer

The second half of the game is what makes it truly meaningful and fun: people.

This includes:

* Cooperation
* Interaction
* Being part of a large economy
* Creating lore
* Building stories
* Forming societies

Since the game is about people, coöperation, and having fun together, the community around the game should play a major role.

## Modding and Customisation

Every part of the game should be built with modding and customization in mind.

This means that the power of a passionate community can be used to create a huge number of solutions, allowing every player
to find a UI, feature, or playstyle that fits them perfectly — or to easily make their own.

## Core Philosophy

A minimal, powerful core that gives players and modders maximum freedom to build layers on top.
